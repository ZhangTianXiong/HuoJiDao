...
@interface TXMediaPlayerMaskView ()<CFDanmakuDelegate>
...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self = [super initWithFrame:frame])
    {
        ...
        //初始化弹幕View
        CFDanmakuView  *  danmakuView  = [[CFDanmakuView alloc]init];
        ...
        [bottomImageView addSubview:barrageBut];
...
-(void)setViewFrame
{
    ...
    CGFloat  barrageButW         = 20;
    CGFloat  barrageButH         = 15;
    CGFloat  barrageButY         = (bottomViewH-barrageButH)/2;
    CGFloat  barrageButX         = margin;
    
    _barrageBut.frame            = CGRectMake(barrageButX, barrageButY, barrageButW, barrageButH);
    
    CGFloat startButX            = CGRectGetMaxX(_barrageBut.frame)+margin;
    ...
    //弹幕Frame
    CGFloat danmakuViewX         = 0;
    CGFloat danmakuViewY         = 2;
    CGFloat danmakuViewW         = CGRectGetWidth(self.bounds);
    CGFloat danmakuViewH         = CGRectGetHeight(self.bounds);
    _danmakuView.frame           = CGRectMake(danmakuViewX, danmakuViewY, danmakuViewW, danmakuViewH);
...
{
    if(self.videoSlider.value ==1.0f)[self.danmakuView stop];
    return self.videoSlider.value*self.totalLength;
}
#pragma mark-------------------弹幕代理-------------------
